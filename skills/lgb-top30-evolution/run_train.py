...
def apply_skill_config(runtime_config: dict, skill_config: dict, version_dir: Path) -> dict:
    ...
    locked_data = skill_config.get("locked_boundaries", {}).get("data", {})

    model_cfg = runtime_config.setdefault("model", {})
    rolling_cfg = model_cfg.setdefault("rolling", {})
    if locked_data.get("valid_start"):
        rolling_cfg["start_date"] = str(locked_data["valid_start"])[:7]
    if locked_data.get("valid_end"):
        rolling_cfg["end_date"] = str(locked_data["valid_end"])[:7]
    ...
    strategy = runtime_config.setdefault("backtest", {}).setdefault("strategy", {})
    ...
    if portfolio_exp.get("mvo_optimizer"):
        runtime_config["backtest"].setdefault("mvo_optimizer", {}).update(portfolio_exp["mvo_optimizer"])
...
def version_dir_for(skill_config: dict) -> Path:
    current = skill_config.get("current", {})
    version = current.get("version", "v000")
    version_dir = Path(__file__).resolve().parent / "versions" / version
    version_dir.mkdir(parents=True, exist_ok=True)
    return version_dir


def render_runtime_config(skill_config: dict) -> tuple[dict, Path]:
    version_dir = version_dir_for(skill_config)
    ...
    return runtime_config, out_path
...
def main() -> int:
    ...
    runtime_config, runtime_path = render_runtime_config(skill_config)
    ...
    write_summary(version_dir_for(skill_config) / "train_summary.json", summary)
